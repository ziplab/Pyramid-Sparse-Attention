...
def flat_group_gemm_kernel(
    ...
    Q_ptrs = Q + batch_id * stride_qz + head_id * stride_qh + block_m * BLOCK_M * stride_ql
    K_ptrs = K + batch_id * stride_kz + head_id * stride_kh + block_n * BLOCK_N * stride_kl

    Q_ptrs = Q_ptrs + tl.arange(0, BLOCK_M)[:, None] * stride_ql + tl.arange(0, BLOCK_K)[
        None, :
    ]
    K_ptrs = K_ptrs + tl.arange(0, BLOCK_N)[None, :] * stride_kl + tl.arange(0, BLOCK_K)[
        :, None
    ]
    ...
    O_ptrs = (
        ...
        + batch_id * stride_oz
        + head_id * stride_oh
        + block_m * BLOCK_M * stride_ol
        + block_n * BLOCK_N
    )
    O_ptrs = O_ptrs + tl.arange(0, BLOCK_M)[:, None] * stride_ol + tl.arange(0, BLOCK_N)[None, :]
...
def softmax_fuse_block_sum(
    ...
    except:  # pragma: no cover - debug fallback
        breakpoint()
